...
class Yolo_test(Evaluator):
    def __init__(self, test_weight):
        ...
        _, _, pred_mbbox, pred_lbbox = Tiny_YOLOV3(training).build_nework(input_data)
        ...
        super(Yolo_test, self).__init__(self.__sess, input_data, training, pred_mbbox, pred_lbbox)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test_weight', help='name of test weights file', default='weights/yolo.ckpt-49-0.8331', type=str)
    ...
    parser.add_argument('-video', help='video detect', default='')
    args = parser.parse_args()

    os.environ['CUDA_VISIBLE_DEVICES'] = args.gpu

    log_time = time.strftime('%Y-%m-%d-%H-%M-%S',time.localtime(time.time()))
    logging.basicConfig(filename='log/test/' + log_time + '.log', format='%(filename)s %(asctime)s\t%(message)s',
    ...
    if  args.video:
        ...
        #print("img_inds_file: {}".format(img_inds_file))
